/**
 * File Tree Formatter Utility
 *
 * Formats GitHub file tree data into a clean, readable directory structure
 */
...
/**
 * Filter tree items to only include files under specified paths
 */
...
/**
 * Filter tree items to limit depth
 * depth=1 means only root level items
 * depth=2 means root and one level down, etc.
 */
...
/**
 * Format tree with filtering by paths and depth
 */
export const formatAsFilteredTree = (
  ...
  // Apply path filtering first
  if (paths && paths.length > 0) {
    filteredFiles = filterByPaths(filteredFiles, paths)
  }

  // Then apply depth filtering
  if (maxDepth && maxDepth > 0) {
